...
class DzsSlidersAdmin{
  ...
  classInit(){
    ...
    // console.log('prepare_send_queue_calls - ', prepare_send_queue_calls);
    ...
    if (queryArg_page == 'slider_multiple') {
      ...
      console.info('_slidersCon - ', $slidersConMultiple);
    ...
    console.info('sliders page - ', queryArg_page);
    ...
    setTimeout(function () {
      ...
      if (queryArg_page == 'slider_single' && get_query_arg(window.location.href, 'taxonomy') == selfInstance.SA_CONFIG['taxonomy']) {
        ...
        // $('#wpbody-content .wrap').eq(0).append($slidersCon);
      ...
      if (e.type == 'change' || e.type == 'keyup') {
        ...
        if ($sliderItem) {
          ...
          for (var lab in window.dzs_slidersAdmin_ajaxQueue) {
            ...
            console.info('val - ', val, id, $t.attr('name'));
            console.groupEnd();
    ...
    function handle_mouse(e) {
      ...
      if (e.type == 'click') {
        ...
        if ($t.hasClass('button-primary')) {
          ...
          // console.warn('window.dzs_slidersAdmin_ajaxQueue-primary - ',window.dzs_slidersAdmin_ajaxQueue, window.dzs_slidersAdmin_ajaxQueue.length);
            ...
            for (var i in response.items) {
              ...
              if (responseItem.type == 'create_item') {
                ...
                if (responseItem.original_request == 'duplicate_item') {
                  console.log('$(\'.slider-item[data-id="\' + cach.original_post_id + \'"]\') -' , $('.slider-item[data-id="' + responseItem.original_post_id + '"]', responseItem.original_post_id, '.slider-item[data-id="' + responseItem.original_post_id + '"]'));
